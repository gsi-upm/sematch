...
def keyphrases_tfidf_ranking(texts, candidates='chunks'):
    import gensim, nltk

    extract = extract_candidate_chunks if candidates == 'chunks' else extract_candidate_words
    phrases = [extract(text) for text in texts]
    phrase_dict = gensim.corpora.Dictionary(phrases)
    corpus = [phrase_dict.doc2bow(p) for p in phrases]
    tfidf = gensim.models.TfidfModel(corpus)
    corpus_tfidf = tfidf[corpus]

    return corpus_tfidf, phrase_dict

def keyphrases_pagerank_ranking(text, n_keywords=0.05):
